...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Request persistent storage for better Android PWA persistence
    if ('storage' in navigator && 'persist' in navigator.storage) {
        navigator.storage.persist().then(persistent => {
            if (persistent) {
                console.log("[PWA] Persistent storage granted - app data will be preserved");
            } else {
                console.log("[PWA] Persistent storage denied - app may be cleared by system");
            }
        }).catch(err => {
    ...
    // Request notification permission to help with Android persistence
    if ('Notification' in window && Notification.permission === 'default') {
        // Only request if not already granted or denied
        Notification.requestPermission().then(permission => {
            if (permission === 'granted') {
                console.log("[PWA] Notification permission granted - helps with app persistence");
            } else {
    ...
    // Estimate storage usage for monitoring
    if ('storage' in navigator && 'estimate' in navigator.storage) {
        navigator.storage.estimate().then(estimate => {
            console.log(`[PWA] Storage usage: ${(estimate.usage / 1024 / 1024).toFixed(2)}MB of ${(estimate.quota / 1024 / 1024).toFixed(2)}MB`);
        });
    }
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.ready.then(registration => {
            if ('sync' in registration) {
                return registration.sync.register('app-maintenance');
            }
        }).then(() => {
            console.log("[PWA] Background sync registered for app maintenance");
        }).catch(err => {
            console.log("[PWA] Background sync not supported:", err);
        });
    }

    // Register periodic background sync if supported (Chrome 80+)
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.ready.then(registration => {
            if ('periodicSync' in registration) {
                return registration.periodicSync.register('app-maintenance', {
            ...
            console.log("[PWA] Periodic background sync not supported:", err);
        });
    }
