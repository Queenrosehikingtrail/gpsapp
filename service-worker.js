// Enhanced service worker for Queen Rose Hiking Trail App - POOR NETWORK OPTIMIZED
const CACHE_NAME = 'queen-rose-hiking-trail-v7-2-network-optimized';
...
self.addEventListener('install', event => {
  console.log('[Service Worker] Installing POOR NETWORK OPTIMIZED version v7.2.0...');
  event.waitUntil(
    ...
    caches.match(event.request)
      .then(cachedResponse => {
        ...
        if (cachedResponse) {
          ...
          // For non-critical resources, try to update cache in background
          if (!requestURL.includes('.html') && !requestURL.includes('.js') && !requestURL.includes('.css')) {
        ...
        // No cache hit - try network with timeout for poor connections
        console.log('[Service Worker] Cache miss, fetching from network with timeout:', requestURL);
        ...
        .catch(error => {
          console.log('[Service Worker] Network fetch failed, checking cache again:', error.message);
          
          // Network failed - try cache one more time (in case it was added during the request)
          return caches.match(event.request)
            .then(fallbackResponse => {
              if (fallbackResponse) {
                console.log('[Service Worker] Found fallback cached response');
