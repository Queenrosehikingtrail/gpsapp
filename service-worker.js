// Enhanced service worker for Queen Rose Hiking Trail App - OPTIMIZED
const CACHE_NAME = 'queen-rose-hiking-v6.1.0-optimized';
...
self.addEventListener('install', event => {
  console.log('[Service Worker] Installing OPTIMIZED version v6.0.0...');
  event.waitUntil(
...
// Background sync for Android PWA persistence
self.addEventListener('sync', event => {
  console.log('[Service Worker] Background sync event:', event.tag);
  if (event.tag === 'background-sync' || event.tag === 'app-maintenance') {
    event.waitUntil(
      // Keep service worker active and maintain app presence
      Promise.resolve().then(() => {
        console.log('[Service Worker] Background sync completed - maintaining app presence');
        // Update cache timestamp to show activity
        return caches.open(CACHE_NAME).then(cache => {
          const timestamp = new Date().toISOString();
          console.log(`[Service Worker] App maintenance sync at ${timestamp}`);
        });
      })
    );
  }
...
// Periodic background sync for better persistence (if supported)
self.addEventListener('periodicsync', event => {
  console.log('[Service Worker] Periodic sync event:', event.tag);
  if (event.tag === 'app-maintenance') {
    event.waitUntil(
      Promise.resolve().then(() => {
        console.log('[Service Worker] Periodic sync completed - app maintenance');
        // Perform lightweight maintenance tasks
        return caches.keys().then(cacheNames => {
          console.log(`[Service Worker] Maintaining ${cacheNames.length} caches`);
        });
      })
    );
  }
...
// Enhanced activation for Android persistence
self.addEventListener('activate', event => {
  console.log('[Service Worker] Activating OPTIMIZED version v6.1.0...');
  ...
  event.waitUntil(
    Promise.all([
      ...
      // Register background sync for persistence
      self.registration.sync ? 
        self.registration.sync.register('background-sync').then(() => {
          console.log('[Service Worker] Background sync registered for persistence');
        }).catch(err => {
          console.log('[Service Worker] Background sync not supported:', err);
        }) : 
        Promise.resolve(),
